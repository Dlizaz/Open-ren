...
function updateProviderUi() {
  ...
  promptLimitNote.innerHTML = isPixazo
    ? `⚠️ X giới hạn mỗi prompt tối đa <strong>${currentPromptMaxLength()} ký tự</strong>.`
...
form.addEventListener("submit", async (e) => {
  ...
  if (missingImageIds.length > 0) {
    formMsg.textContent = `Dòng ${missingImageIds.join(", ")} chưa có ảnh - X bắt buộc phải có ảnh đầu vào.`;
    return;
  }
...
function renderTable(items) {
  ...
  for (const id of ids) {
    ...
    const providerLabel = item.provider === "pixazo" ? "X" : "Runway";
