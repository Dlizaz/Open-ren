...
async function processItem(item) {
  ...
  if (item.prompt && item.prompt.length > promptMaxLength) {
    await updateItemStatus(item.id, {
      state: "FAILED",
      error: `promptText: Prompt dài ${item.prompt.length} ký tự, vượt quá giới hạn ${promptMaxLength} ký tự của ${provider === "pixazo" ? "Pixazo" : "Runway"}`,
      errorCode: "too_big",
  ...
  if (provider === "pixazo" && !item.imageUrl) {
    await updateItemStatus(item.id, {
      state: "FAILED",
      error: "Pixazo (LTX) bắt buộc phải có ảnh đầu vào (chỉ hỗ trợ image-to-video)",
  ...
  while (attempt <= MAX_RETRIES) {
    try {
      ...
      if (!videoUrl) throw new Error(`${provider === "pixazo" ? "Pixazo" : "Runway"} render xong nhưng không trả về video URL`);
