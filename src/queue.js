...
async function processItem(item) {
  ...
  if (provider === "pixazo" && !item.imageFileId && !item.imageUrl) {
    await updateItemStatus(item.id, {
      state: "FAILED",
  ...
  while (attempt <= MAX_RETRIES) {
    try {
      ...
      try {
        if (provider === "pixazo") {
          // Ghep domain HIEN TAI ngay luc nay (khong dung link co dinh da
          // luu tu luc upload) - neu item cu (truoc ban vá nay) da co san
          // imageUrl day du thi van dung tam, uu tien imageFileId moi.
          const imageUrl = item.imageFileId
            ? `${getPublicBaseUrl()}/uploads/${item.imageFileId}`
            : item.imageUrl;
