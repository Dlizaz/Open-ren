...
// Thoi gian cho toi da truoc khi tu bo (phut). Mac dinh 30 phut - qua theo
// doi log thuc te, nhieu job Pixazo hoan thanh binh thuong nhung mat toi
// 15-18 phut (khong lien quan gi den field duration), nen 4-8 phut la qua
// ngan, gay huy oan nhung job dang chay tot. Chinh duoc qua bien moi truong
// PIXAZO_POLL_TIMEOUT_MINUTES tren Railway neu can dieu chinh them.
const POLL_TIMEOUT_MS = (Number(process.env.PIXAZO_POLL_TIMEOUT_MINUTES) || 30) * 60 * 1000;
...
export function describePixazoError(error) {
  ...
  return {
    message: error?.message || "Loi khong xac dinh tu X",
...
async function submitJob({ prompt, imageUrl, duration, sendDuration }) {
  ...
  // HOAC khi bien moi truong PIXAZO_SEND_DURATION=true duoc set (bat mac
  // dinh cho ca app). Ly do phai co cong tac rieng: da co bang chung (so
  // sanh voi script test cu chi gui prompt+image_url) rang gui them field
  // nay CO THE khien job bi treo im lang, nen mac dinh phai la TAT.
  const envForce = process.env.PIXAZO_SEND_DURATION === "true";
  const shouldSendDuration = Boolean(sendDuration) || envForce;
...
async function pollUntilDone(pollingUrl, abortSignal) {
  ...
  while (Date.now() < deadline) {
    ...
    console.log(`X poll (${elapsedSec}s): status=${data.status}`);

    if (data.status === "COMPLETED") return data;
    if (data.status === "FAILED" || data.status === "ERROR") {
      const err = new Error(`X job that bai: ${JSON.stringify(data)}`);
      err.pixazoCode = data.status;
      throw err;
    }
  }

  const timeoutMinutes = POLL_TIMEOUT_MS / 60000;
  throw new Error(`Qua ${timeoutMinutes} phut X van chua xong, tu bo cho.`);
}
 ...
 */
export async function generatePixazoVideo({ prompt, imageUrl, duration, sendDuration, abortSignal, onTaskCreated }) {
  assertConfigured();

  if (!imageUrl) {
    throw new Error("X bắt buộc phải có ảnh đầu vào (chỉ hỗ trợ image-to-video)");
  }

  const submitResult = await submitJob({ prompt, imageUrl, duration, sendDuration });
  ...
  if (!videoUrl) {
    throw new Error("X render xong nhưng không trả về video URL");
  }
