...
app.post("/api/item/:id/delete", async (req, res) => {
  try {
    ...
    if (item?.imageFileId || item?.imageUrl) {
      const imageFileId =
        item.imageFileId || item.imageUrl.match(/\/uploads\/([a-fA-F0-9]+)$/)?.[1];
      if (imageFileId) {
        try {
          const bucket = await getImageBucket();
          await bucket.delete(new ObjectId(imageFileId));
        } catch {
          // Da bi xoa tu truoc hoac khong ton tai, bo qua.
        }
      }
...
/**
 * Luu 1 anh (buffer) vao GridFS bucket "images", tra ve CHI fileId (KHONG
 * ghep domain thanh URL day du o day nua). Domain se duoc ghep vao ngay
 * LUC CHUAN BI GUI cho Pixazo (xem queue.js + publicUrl.js), de neu ban doi
    ...
    try {
      ...
      for (const raw of rawItems) {
        ...
        if (imageBuffer) {
          ...
          if (provider === "pixazo") {
            // Chi luu vao GridFS (lay fileId) khi thuc su dung Pixazo.
            imageFileId = await storeImage(imageBuffer, raw.imageName);
          }
        }
